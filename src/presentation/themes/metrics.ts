...
import {
  ...
  isiOS,
  isiPhoneX,
} from '@infrastructure/utils/platformUtils';
...
const shouldScaleDown = scaleCoefficient <= 1;
const scaleDown = Math.min(width / dw, height / dH);
const scaleUp = Math.max(dw / width, dH / height);
const scaleCoefficientDown = scaleDown > 1 ? 1 : scaleDown;
const scaleCoefficientUp = scaleUp < 1 ? 1 : scaleUp;
const scaleCoefficientUpProportional = (scale: number) =>
  scaleUp < 1 ? 1 : scaleUp * scale;
...
export default {
  ...
  scaleCoefficient,
  shouldScaleDown,
  scaleCoefficientDown,
  scaleCoefficientUp,
  scaleCoefficientUpProportional,
  horizontal,
  vertical,
  scale,
  scaleY,
