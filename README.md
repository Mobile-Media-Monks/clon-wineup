...
`[{SP}-{number}]_[short-description]`

- **`[{SP}-{number}]`**: A reference to the related task or ticket, where `{SP}` stands for the project or sprint identifier (e.g., `SP-`, `APP-`, or a custom identifier), and `{number}` is the ticket number.
...
- Open the `src/domain/theme/typography.ts` file and add the new font configuration:
